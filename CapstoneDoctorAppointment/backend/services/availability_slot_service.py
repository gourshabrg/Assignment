...
class AvailabilitySlotService:
    """Business logic for a doctor managing their own bookable slots."""
    ...
    def _validate_slot_time(self, slot_date: date, start_time, end_time):
        """Rejects past dates and start >= end time ranges."""
    ...
    async def get_my_slots(
        ...
        try:
            ...
            return ApiResponse(
                ...
                data=[self._build_response(slot) for slot in slots]
            )
    ...
    async def update_slot(
        ...
        """Doctor updates a slot they own, unless it's already booked."""
        ...
        try:
            ...
            if slot.is_booked:
                logger.warning(
                    f"Slot update failed: already booked slot_id={slot_id}"
                )
    ...
    async def delete_slot(
        ...
        """Doctor deletes a slot they own, unless it's already booked."""
        ...
        try:
            ...
            if slot.is_booked:
                logger.warning(
                    f"Slot delete failed: already booked slot_id={slot_id}"
                )
                raise SlotAlreadyBookedException()
